...
export default function AuthPage() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      console.log(`Starting auth process: ${isLogin ? 'Login' : 'Signup'}`);
      if (isLogin) {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center p-4 bg-slate-50">
      ...
      <Card className="w-full max-w-md border-none shadow-2xl rounded-[2.5rem] overflow-hidden bg-white">
        <CardHeader className="space-y-1 pt-12 pb-8 text-center bg-slate-900 text-white">
          ...
          <CardTitle className="text-3xl font-black tracking-tight uppercase">
            {isLogin ? "Welcome Back" : "Join the Market"}
          </CardTitle>
          <CardDescription className="text-slate-400 font-medium">
            {isLogin 
              ? "Access your institutional-grade collection alpha." 
              : "Start tracking your assets with precision analytics."}
          </CardDescription>
        </CardHeader>
        <CardContent className="p-8 pt-10">
          <Button 
            variant="outline"
            className="w-full h-14 border-slate-200 rounded-2xl font-bold flex items-center justify-center gap-3 hover:bg-slate-50 transition-all active:scale-95 mb-6"
          ...
          <form onSubmit={handleAuth} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input 
                    id="name" 
                    placeholder="Red Ash" 
                    className="pl-12 h-14 rounded-2xl border-slate-100 bg-slate-50 focus-visible:ring-red-600 font-bold"
                    value={fullName}
            ...
            <Button 
              className="w-full h-14 bg-red-600 hover:bg-red-700 text-white rounded-2xl font-black text-lg transition-all active:scale-95 shadow-xl shadow-red-100" 
              disabled={isLoading}
            >
              {isLoading ? (
          ...
          <div className="mt-8 text-center">
            <button 
              ...
              className="text-sm font-black text-slate-400 hover:text-red-600 uppercase tracking-widest transition-colors"
            >
